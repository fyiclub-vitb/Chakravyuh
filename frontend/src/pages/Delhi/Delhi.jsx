...
const Delhi = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    const correctPasscode = '999'
    const enteredCode = code.join('')

    if (enteredCode === correctPasscode) {
      setMessage('Mission Complete! All cities cleared!')
      try {
        const userData = JSON.parse(localStorage.getItem('userData') || '{}')
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-black">
      <div className="w-4/5 h-full">
        <iframe src="/game/index.html" className="w-full h-full border-none" title="Delhi Game" />
      </div>
      ...
      <div className="w-1/5 h-full bg-[#0b0f1a] text-white flex flex-col border-l border-gray-800 overflow-y-auto">
        ...
        <div className="px-6 py-6 border-b border-gray-800">
          <p className="text-[11px] tracking-widest text-purple-400 font-semibold mb-3">FINAL TASK</p>
          <div className="bg-[#121826] p-4 rounded-lg border border-purple-500/30">
            <p className="text-base font-semibold text-white">Locate Final 3-Digit Code</p>
            <p className="text-xs text-gray-400 mt-1">This is the last checkpoint. Complete the mission!</p>
          </div>
        </div>
        ...
        <div className="px-6 py-6 mt-auto bg-[#0b0f1a] border-t border-gray-800">
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-[10px] text-gray-500 tracking-widest">3-DIGIT ACCESS</span>
          </div>
            ...
            <div className="flex justify-center gap-4">
              {[0, 1, 2].map((i) => (
                <input key={i} ref={inputRefs[i]} type="text" inputMode="numeric" maxLength={1}
                  ...
                  className="w-16 h-16 bg-[#0b0f1a] border border-gray-700 rounded-lg text-center text-2xl font-mono text-white transition focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20"
                  autoComplete="off" />
              ))}
            </div>
